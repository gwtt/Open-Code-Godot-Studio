...
### 4. 场景定义结构性节点 (Scene-Defined Structural Nodes)

**规则**: 所有结构性子节点必须在 .tscn 场景文件中定义，禁止在 `_ready()` 中通过代码动态创建。

| 节点类型 | 正确做法 | 禁止做法 |
|----------|---------|---------|
| CollisionShape2D/3D | MCP godot_add_node → .tscn | ❌ CollisionShape2D.new() + add_child() |
| Sprite2D/3D | MCP godot_add_node → .tscn | ❌ Sprite2D.new() + add_child() |
| Camera2D/3D | MCP godot_add_node → .tscn | ❌ Camera2D.new() + add_child() |
| AnimationPlayer | MCP godot_add_node → .tscn | ❌ AnimationPlayer.new() + add_child() |
| Timer | MCP godot_add_node → .tscn | ❌ Timer.new() + add_child() |

**允许的动态创建** (PackedScene.Instantiate()):
- 敌人/子弹/粒子等运行时动态实体

**禁止行为**:
- ❌ 在 `_ready()` 中 add_child() 创建结构性节点
- ❌ 在 `_init()` 中动态创建永久场景架构节点

**正确做法**:
- ✅ 使用 MCP 工具 (godot_add_node) 在 .tscn 中创建节点
- ✅ 使用 @onready / [Export] GetNode 引用场景节点
- ✅ 节点属性通过 Inspector 可调，在编辑器中可见
...
## Available Skills (22) - 3-Tier Architecture

### Tier 1: Directors (战略决策)
`creative-director` `technical-director` `producer` `start` `resume`

**Role**: 项目方向、范围控制、技术愿景

**resume**: 新会话快速恢复项目上下文，无需重新走 /start 流程
